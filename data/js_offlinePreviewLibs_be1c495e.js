...
var DPMainToolbar = new MooTools.Class({
  ...
  showShareModal: function(templateId) {
    ...
    this.mShareModal = new ShareProjectModal({
      ...
      idCollection: dpManager.dpProject.getIdCollection(),
    });
    ...
    if (uxpft.enabled(FEATURE_SLACK)) {
      this.mShareModal.hideContent();
      SlackService.getState().then(function (data) {
        this.mShareModal.initializeComponents(data.authorized);
        this.mShareModal.showContent();
      }.bind(this));
    } else {
      this.mShareModal.initializeComponents(false);
    }
